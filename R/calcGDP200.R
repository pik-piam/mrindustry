#' Get GDP from 1900-2100
#' @description
#' Calc GDP (PPP) from 1900-2100 yearly for the SIMSON format on a country 
...
#' @export
calcGDP200 <- function(scenario='SSP2', per_capita=FALSE) {
  # load data
  data <- getGDP200Data(scenario=scenario)
  
  # interpolate
  data <- interpolateGDP200(data)
  
  # convert per capita to total
  data$hist <- data$pop[,1:117] * data$hist_pc
  
  # extrapolate
  data <- extrapolateGDP200(data)
  ...
  unit <- '2005 USD$PPP'
  description='GDP from 1900-2100 yearly for the SIMSON format'
  weight<-NULL
  ...
  if (per_capita) {
    ...
    description='GDP per capita from 1900-2100 yearly for the SIMSON format'
    weight <- data$pop
  } 
...
getGDP200Data <- function(scenario){
  # load data
  pop <- calcOutput('Population200', aggregate=F)
...
interpolateGDP200 <- function(data) {
  # interpolate
  data$recent <- time_interpolate(data$recent, 1965:2100)
...
extrapolateGDP200 <- function(data) {
