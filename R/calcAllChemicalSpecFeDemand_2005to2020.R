...
calcAllChemicalSpecFeDemand_2005to2020 <- function(CCS=FALSE) {
  ...
  specFeDem <- IEA_PetrochemEI %>% # in GJ/t
    ...
    mutate(Value_new =
             case_when(.data$Data1=="Steam cracking, fuel & steam" & .data$Value>0 ~ .data$Value+(58-.data$Value_min),    # Source: rough average of 66 GJ/t of Spallina17 Table 5, 53 GJ/t Yang17 Table 1 and 53 GJ/t Layritz21 Table A2; Value for naptha steam cracking, as the region with the lowest energy intensity JPN has 96% naphtha steam crackers (Saygın et al. 2009. Chemical and Petrochemical Sector.Table 11)
                       .data$Data1=="Methanol, fuel & steam"  & .data$Value>0 ~ .data$Value+(33.9-.data$Value_min),        # Source: IEA, The Future of Hydrogen19 PAGE | 5; Value the natural gas based route as this route is employed in regions with lowest energy intensity
                       TRUE ~ .data$Value))
  ...
  demFeRatio <- merge(demFeTarget, demFeActual, by=c("Region","Year", "Data1")) %>%
    ...
    mutate(demFeRatio = case_when(.data$demFeRatio<1 ~ 1, TRUE ~ .data$demFeRatio)) # regional energy intensity cannot be lower than the target (possibly coal-based methanol in IND and OAS in AllChemicalRoutes_2020higher than the one assumed by IEA_PetrochemEI)
  ...
  OtherChem_FE <- feIndustry %>%
    ...
    mutate(
      ...
      Total_Difference = .data$feChemicals - .data$Total_Energy_Demand
    ) %>%
